...
sub coercion {
    ...
    Moose::Meta::TypeCoercion->new(
        type_coercion_map => [ $params{'from'} => $params{'via'} ] );
}
...
sub verify {
    ...
    foreach my $key ( keys( %{$profile} ) ) {
        ...
        if ( $fprof->{type} ) {
            ...
            die "Unknown type constraint '$fprof->{type}'"
              unless defined($cons);
            ...
            # whole profile.
            if (   $cons->is_a_type_of('ArrayRef')
                && $cons->can('type_parameter') )
            {
                ...
                foreach my $poss (@possibles) {
                    ...
                    if ( $res->success ) {
                        ...
                        push( @pass_post_filter,
                            $res->get_post_filter_value($key) );
                    }
                    else {
        ...
        if (   $members
            && defined( $fprof->{member_post_check} )
            && $fprof->{member_post_check} )
        {
            push( @post_checks, $key );
        }
    ...
    if ( scalar(@post_checks) ) {
        foreach my $key (@post_checks) {
            ...
            # plain ol' post check.
            my $pc =
              $members ? $fprof->{member_post_check} : $fprof->{post_check};
            if ( defined($pc) && $pc ) {
        ...
        foreach my $key ( keys( %{ $self->derived } ) ) {
            ...
            if ( defined($req) && $req && !defined($rv) ) {
                ...
                foreach my $df ( @{$dfields} ) {
                    ...
                    die "Unknown field '$df' in derived field '$key'!"
                      unless defined $f;
                    $f->valid(0);
                    $f->value(undef);
                    $f->reason('derived');
